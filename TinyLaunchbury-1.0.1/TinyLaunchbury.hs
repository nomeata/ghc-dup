...
import Data.List(foldl',intercalate,nub,(\\),delete)
...
instance Show Expr where
  ...
  show (Int i) = show i
  show (IfZero e1 e2 e3) = "if " ++ show e1 ++ "==0 then " ++ show e2 ++ " else " ++ show e3
  show (Sub1 x) = x ++ " - 1"
...
sub x y e =
 ...
 in case e of
       ...
       Let bs e'  | elem x (binders bs) -> e -- only want to sub free variables;
                                             -- x is no longer free
                  | otherwise           -> Let (map subBn bs) (subExpr e')
       ...
       Int n                            -> Int n
       Sub1 z                           -> Sub1 (subName z)
       IfZero e1 e2 e3                  -> IfZero (subExpr e1) (subExpr e2) (subExpr e3)
       DeepDup z                        -> DeepDup (subName z)
       Dup z                            -> Dup (subName z)

freeVarNames :: Expr -> [Name]
freeVarNames (Lambda z e') = delete z $ freeVarNames e'
freeVarNames (Apply e' z) = nub $ z : freeVarNames e'
freeVarNames (Var z) = [z]
freeVarNames (Let bs e) = (nub $ concatMap (freeVarNames . snd) bs ++ freeVarNames e) \\ map fst bs
freeVarNames (Dup x) = [x]
freeVarNames (DeepDup x) = [x]
freeVarNames (Prim fun e' e'') = nub $ freeVarNames e' ++ freeVarNames e''
freeVarNames (Ctor _ args) = nub args
freeVarNames (Case e alts) = nub $ freeVarNames e ++ concatMap freeVarNamesAlt alts
freeVarNames (Int n) = []
freeVarNames (Sub1 z) = [z]
freeVarNames (IfZero e1 e2 e3) = nub $ freeVarNames e1 ++ freeVarNames e2 ++ freeVarNames e3

freeVarNamesAlt :: Alt -> [Name]
freeVarNamesAlt (ctr, (ns, e)) = freeVarNames e \\ ns
...
freshen c@(Int _)          = return c
freshen c@(Sub1 _)         = return c
freshen c@(IfZero e1 e2 e3)= do e1' <- freshen e1
                                e2' <- freshen e2
                                e3' <- freshen e3
                                return $ IfZero e1' e2' e3'
freshen c@(DeepDup _)      = return c
freshen c@(Dup _)          = return c
...
reduceM e = let logCase msg = do s <- get
 ...
 in case e of
        ...
        Dup x -> do logCase "Reducing dup: "
                    z <- heapLookup x
                    x' <- getFreshVar
                    heapAdd x' z
                    withLogIndent $ reduceM (Var x')
        DeepDup x -> do logCase "Reducing deepDup: "
                        z <- heapLookup x
                        case z of 
                            DeepDup y -> withLogIndent $ reduceM (DeepDup y)
                            _ -> do
                                x' <- getFreshVar
                                let fv = freeVarNames z
                                fv' <- getFreshVars (length fv)
                                thunks <- forM (zip fv fv') $ \(v,v') -> do
                                    e'<- heapLookup v
                                    heapAdd v' $ DeepDup v
                                let z' = subs (zip fv fv') z
                                heapAdd x' z'
                                withLogIndent $ reduceM (Var x')
        ...
        Int n -> do logCase "Returning Integer: "
                    return $ Int n

        Sub1 x -> do logCase "Reducing Sub1: "
                     Int n <- withLogIndent $ reduceM (Var x)
                     return $ Int (n-1)

        IfZero e1 e2 e3 ->
            do logCase "Reducing IfZero: "
               Int n <- withLogIndent $ reduceM e1
               if (n == 0)
                  then withLogIndent $ reduceM e2
                  else withLogIndent $ reduceM e3
...
enumTest = Let
    [("enum", Lambda "n" $ IfZero (Var "n")
        (Ctor 0 [])
        (Let [
            ("n-1", Sub1 "n"),
            ("tail", Apply (Var "enum") "n-1")
            ] $ Ctor 1 ["n", "tail"] )),
      ("0", Int 0),
      ("5", Int 5),
      ("walk", Lambda "n'" $ Lambda "e" $ Case (Var "e") [
        (0, ([], Var "n'")),
        (1, (["h","t"], Let [("n'-1", Sub1 "n'")] $
            Apply (Apply (Var "walk") "n'-1") "t"))]),
      ("list", Apply (Var "enum") "5")
    ] $ Apply (Apply (Var "walk") "0") "list"
...
enumTest2 = Let
    [("enum", Lambda "n" $ IfZero (Var "n")
      ...
      ("0", Int 0),
      ("5", Int 5),
      ("walk", Lambda "n'" $ Lambda "e" $ Case (DeepDup "e") [
        (0, ([], Var "n'")),
        (1, (["h","t"], Let [("n'-1", Sub1 "n'")] $
            Apply (Apply (Var "walk") "n'-1") "t"))]),
      ("list", Apply (Var "enum") "5"),
