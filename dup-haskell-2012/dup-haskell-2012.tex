...
In addition to the unmodified reduction rules \sRule{Lam}, \sRule{App}, \sRule{Variable} and \sRule{Let}, we add the two rules \sRule{Dup} and \sRule{Deep} in Figure \ref{fig:semrules}. In these rules, $\fv e$ denotes the set of free variables of $e$, $\hat z$ is $z$ with all bound variables renamed to completely fresh variables and the variables $x'$ and $y_i'$ are fresh. In the following every heap/term pair $\Gamma : e$ is assumed to be distinctly named, i.e.\ every binding in $\Gamma$ and in $e$ binds a distinct variable; this property is preserved by the reduction rules.
...
Besides the natural semantics, Launchbury also defines a denotational semantics. He models values as a lifted function space and environments as functions from variables into values. He writes $\rho \le \rho'$  if $\rho'$ extends $\rho$. The expression $\dsem e \rho$ is the value of the expression $e$ in the environment $\rho$, $\esem \Gamma \rho$ is the environment $\rho$ updated by the values specified in the heap $\Gamma$. This is defined as a fixed point, as the heap may contain recursive references:
\begin{multline*}
...
\begin{lemma}
\label{lem:deepdup}
Let $\Gamma_0$ be a heap and $U= \dom\Gamma_0$ its domain. If $\sred\Gamma e \Delta z$, then $\Gamma_0 \subseteq \Gamma$ and $U \cap \ur \Gamma e = \emptyset$ implies
 $\Gamma_0 \subseteq \Delta$ and $U \cap \ur \Delta z=\emptyset$. 
\end{lemma}

To be able to prove this lemma, we need another technical property:
\begin{lemma}
If $\sred\Gamma e \Delta z$, $\Gamma_0 \subseteq \Gamma$ and $U \cap \ur\Gamma e = \emptyset$ then $U \cap \ur\Gamma y = \emptyset$ implies $U\cap \ur\Delta y = \emptyset$ for every $y\in \dom\Gamma$.
\label{lem:varpres}
\end{lemma}

Both lemmas need to be proven simultaneously and by induction on the structure of the derivation $\sred\Gamma e \Delta z$. We give the two proof separately nevertheless and pay attention to apply the other lemma only on the hypothesizes of the current judgment.

\begin{proof}[of Lemma \ref{lem:deepdup}]
\case{$\sLam xe$}
Immediate.
\case{$\sApp e x$}
From $\ur\Gamma{\sApp e x} = \ur\Gamma{e} \cup \ur\Gamma{x}$ and the assumption $U \cap \ur\Gamma{\sApp e x}= \emptyset$ we have $U \cap \ur\Gamma e = \emptyset$ and $U \cap \ur\Gamma x = \emptyset$. From the first induction hypothesis we obtain $\Gamma_0 \subseteq \Delta$, (i) $U \cap \ur\Delta {\sLam y e'}= \emptyset$ and, by invoking Lemma \ref{lem:varpres} on $x$, (ii) $U \cap \ur\Delta{x} = \emptyset$.

As $\ur\Delta{e'[x/y]} \subseteq \ur\Delta{\sLam ye'} \cup \ur\Delta{x}$, (i) and (ii) imply $U \cap \ur\Delta{e'[x/y]} = \emptyset$. With $\Gamma_0 \subseteq \Delta$ we obtain $\Gamma_0\subseteq\Theta$ and $U\cap \ur\Theta z = \emptyset$ from the second induction hypothesis.
\case{$x$}
As $x \in \ur{\Gamma,x \mapsto e} x$ and removing a variable from a heap does not increase unreachable sets, $\ur\Gamma e \subseteq \ur{\Gamma, x\mapsto e}e \subseteq \ur{\Gamma,x\mapsto e} x$. Furthermore, $x \notin U$, so $\Gamma_0 \subseteq \Gamma$ and we can invoke the induction hypothesis to obtain $\Gamma_0 \subseteq \Delta$ and $U \cap \ur\Delta z = \emptyset$. As $\Delta \subseteq (\Delta, x \mapsto z)$, $\ufv{z} = \ufv{\hat z}$ and $\ur \Delta z = \ur {\Delta, x \mapsto z} {\hat z}$, the statement follows.
...
&\subseteq \mu V. V
\begin{aligned}[t]
&\cup \ufv {e_l} \cup \{x_1,\ldots,x_n\} \cup \textstyle\bigcup_{x\in \ufv {e_l}}\ur{\Gamma'}{\Gamma'\ x} \\
...
\end{aligned}\\
&= \mu V. V \cup \ufv {e_l} \cup \{x_1,\ldots,x_n\} \cup \textstyle\bigcup_{x\in \ufv {e_l}}\ur{\Gamma'}{\Gamma'\ x}\\
...
\begin{proof}[of Lemma \ref{lem:varpres}]
\case{$\sLam xe$}
Immediate.
\case{$\sApp e x$}
Follows immediately from the inductive hypothesizes.
\case{$x$}
As $\ur\Gamma y \subseteq \ur{\Gamma,x\mapsto e} y$ holds we obtain $U\cap\ur{\Delta}x=\emptyset$ from the induction hypothesis and  $U\cap \ur{\Delta} z = \emptyset$ from Lemma \ref{lem:deepdup} applied to the inductive case $\sred\Gamma e \Delta z$. Together with $\ur{\Delta,x\mapsto z}y \subseteq \ur{\Delta}y \cup \ur{\Delta}z$ this implies the statement.
\case{$\sLet {x_1=e_1,\ldots,x_n=e_n}e$}
For $y\in \dom\Gamma$ the unreachable set of $y$ cannot contain any of $x_1,\ldots,x_n$, as heaps are distinctly named, so we have $\ur\Gamma{y} = \ur{\Gamma'}y$ and the statement follows by induction.
\case{$\sDup x$ and $\sDeepDup x$}
The statement follows immediately as the added variables are fresh.
\end{proof}
