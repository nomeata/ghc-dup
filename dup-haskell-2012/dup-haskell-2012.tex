...
\begin{lemma}
If $\sred\Gamma e \Delta z$, then $U \cap \ur\Gamma y = \emptyset$ implies $U\cap \ur\Delta y = \emptyset$ for every $y\in \dom\Gamma$.
\label{lem:varpres}
\end{lemma}
...
Immediate.
\case{$x$}
If $x \notin \ur{\Gamma,x\mapsto e} y$, then $\ur{\Gamma,x\mapsto e} y = \ur{\Gamma} y$ and from the induction hypothesis we know $U\cap\ur{\Delta}y=\emptyset$ and hence $U\cap\ur{\Delta,x\mapsto z} y =\emptyset$.

If $x\in \ur{\Gamma,x\mapsto e}y$, then $\ur{\Gamma,x\mapsto e} y = \ur{\Gamma} y \cup \ur{\Gamma,x\mapsto e}x$ and the statement follows from the induction hypothesis and Lemma \ref{lem:deepdup} applied to the inductive case.
\case{$\sLet {x_1=e_1,\ldots,x_n=e_n}e$}
For $y\in \dom\Gamma$ the unreachable set of $y$ cannot contain any of $x_1,\ldots,x_n$ so from $\ur\Gamma{y} = \ur{\Gamma'}y$ the statement follows by induction.
