...
\begin{lemma}
\label{lem:deepdup}
Let $\Gamma_0$ be a heap and $U= \dom\Gamma_0$ its domain. If $\sred\Gamma e \Delta z$, then the following two statements hold:
\begin{itemize}
\item $\Gamma_0 \subseteq \Gamma$ and $U \cap \ur \Gamma e = \emptyset$ implies
 $\Gamma_0 \subseteq \Delta$ and $U \cap \ur \Delta z=\emptyset$ and
\item $U \cap \ur\Gamma x = \emptyset$ implies $U\cap \ur\Delta x = \emptyset$ for $x\in \dom\Gamma$.
\end{itemize}
\end{lemma}
\begin{proof}
...
Immediate.
\case{$\sApp e x$}
From $\ur\Gamma{\sApp e x} = \ur\Gamma{e} \cup \ur\Gamma{x}$ and the assumption $U \cap \ur\Gamma{\sApp e x}= \emptyset$ we have $U \cap \ur\Gamma e = \emptyset$ and $U \cap \ur\Gamma x = \emptyset$. From the first inductive case we obtain $\Gamma_0 \subseteq \Delta$, (i) $U \cap \ur\Delta {\sLam y e'}= \emptyset$ and (ii) $U \cap \ur\Delta{x} = \emptyset$.

As $\ur\Delta{e'[x/y]} \subseteq \ur\Delta{\sLam ye'} \cup \ur\Delta{x}$, (i) and (ii) imply $U \cap \ur\Delta{e'[x/y]} = \emptyset$. With $\Gamma_0 \subseteq \Delta$ we obtain $\Gamma_0\subseteq\Theta$ and $U\cap \ur\Theta z = \emptyset$ from the second inductive case.

These second statement of Lemma \ref{lem:deepdup} follows directly by inductively.
\case{$x$}
As $x \in \ur{\Gamma,x \mapsto e} x$ and removing a variable from a heap does not increase unreachable sets, $\ur\Gamma e \subseteq \ur{\Gamma, x\mapsto e}e \subseteq \ur{\Gamma,x\mapsto e} x$. Furthermore, $x \notin U$, so $\Gamma_0 \subseteq \Gamma$ and we can invoke the inductive case to obtain $\Gamma_0 \subseteq \Delta$ and $U \cap \ur\Delta z = \emptyset$. As $\Delta \subseteq (\Delta, x \mapsto z)$, $\ufv{z} = \ufv{\hat z}$ and $\ur \Delta z = \ur {\Delta, x \mapsto z} {\hat z}$, the  first statement follows.

The second statement follows directly by induction for variables other than $x$; for $x$ it follows from the first statement.
...
= \emptyset$, we have $U \cap \ur {\Gamma'} e = \emptyset$ and the first statement
follows from the inductive case.

For $x\in \dom\Gamma$ the unreachable set of $x$ cannot contain any of $x_1,\ldots,x_n$ so from $\ur\Gamma{x} = \ur{\Gamma'}x$ the second statement follows by induction.
\case{$\sDup x$}
...
\end{align*}
As $x'$ is fresh, $\fresh x\notin U$ and from $U \cap \ur{\Gamma, x\mapsto e}{\sDup x}= \emptyset$ we have $U \cap \ur{\Gamma,x\mapsto e, \fresh x \mapsto \hat e}{\fresh x}=\emptyset$, so the first statement follows from the inductive case.

The second statement follows immediately as $\fresh x$ is fresh.
\case{$\sDeepDup x$}
...
\end{align*}
and, as these are all fresh variables, $U \cap \ur{\Gamma'}{\fresh x} = \emptyset$. Clearly, $\Gamma_0 \subseteq (\Gamma, x\mapsto e) \subseteq \Gamma'$, so the  first statement follows from the inductive case.

The second statement follows immediately as the added variables are fresh.
\end{proof}
