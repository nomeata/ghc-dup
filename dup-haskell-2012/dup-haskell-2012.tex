...
\sffamily
\begin{tikzpicture}
[level/.style={sibling distance=22mm/2^#1,level distance=6mm},
...
\end{haskell}
Now we achieve the performance of a successful run with \li-dup- (\stats{stats:SolveDeepDup:SharedThunk:mem}~MB and \stats{stats:SolveDeepDup:SharedThunk:time} seconds), but also in the cases where \li-t- has already been partly evaluated or wrapped in another unevaluated expression. The second row of Figure \ref{fig:deepevaluation} shows \li-deepDup- at work.
...
As we can see from the statistics in Figure \ref{fig:statistics}, the unit type argument pattern is the clear winner in both run-time and space performance. It is ahead of \li-rateDup- for the same reason that made \li-rateDup- faster than \li-solveDup-: Now even the subtrees in recursive calls of \li-rate- are freed immediately.
Unfortunately, it requires a thorough refactoring of both the data generating and data consuming code; all combinators working on the data type need to be carefully rewritten to preserve the non-sharing behaviour of the lifted data type. Also, the full laziness transformation can break the pattern, making it slightly fragile.

The church encoding pattern shows good and predicatable memory performance, but exhibits bad run-time behaviour. The cases where it is ahead of other approaches it wins only due to the garbage collector overhead induced by unprevented sharing. As the previous pattern, it requires extensive refactoring.

Our primitives come with a very small overhead when applied to data that is actually unshared, as shown in the first colunmn. In fact, careful use of \li-dup- can improve performance noticably even if only small pieces of data can be un-shared and thus freed quickly.
...
In addition to the unmodified reduction rules \sRule{Lam}, \sRule{App}, \sRule{Variable} and \sRule{Let}, we add the two rules \sRule{Dup} and \sRule{Deep} in Figure \ref{fig:semrules}. In these rules, $\fv e$ is the set of free variables of $e$, $\hat z$ is $z$ with all bound variables renamed to completely fresh variables and the variables $x'$, $y_i'$ are fresh..
...
Besides the natural semantics, Launchbury also defines a denotational semantics He models values as a lifted function space and environments as maps from variables into values. The expression $\dsem e \rho$ is the value of the exression $e$ in the environment $\rho$; $\esem \Gamma \rho$ is the environment $\rho$ updated by the values specified in the heap $\Gamma$ using a fixed poined, as the heap may contain recursive references:
\begin{multline*}
...
\case{$\sDup x$}
Clearly $\Gamma_0 \subseteq \Gamma, x\mapsto e \subseteq \Gamma, x\mapsto e, \fresh x \mapsto e$. Also, $\ur{\Gamma,x\mapsto e, \fresh x\mapsto e}{\fresh x} = \ur{\Gamma, x\mapsto e}{x} \cup \{x\} = \ur{\Gamma, x\mapsto e}{x} \cup \{\sDup x\} $. As all bound variables are distinct, $\fresh x\notin U$ and from $U \cap \ur{\Gamma, x\mapsto e}{\sDup x}= \emptyset$ we have $U \cap \ur{\Gamma,x\mapsto e, \fresh x \mapsto e}{\sDup x}=\emptyset$, so the statement follows from the inductive case.
\case{$\sDeepDup x$}
...
While Haskell gives the programmer great devices to get their programs to do the right thing, such as referential transparency and the type system, she has less ways to analize and control their runtime behaviour. Several commercial users of Haskell have mentioned this as one of the main drawbacks of Haskell \citep{sampson,wehr,hesselink}. This work takes one small step towards adressing these issues.
