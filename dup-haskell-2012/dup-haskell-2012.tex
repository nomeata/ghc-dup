...
\authorinfo{Joachim Breitner}{Karlsruhe Institute for Technology}{breitner@kit.edu}
...
\begin{abstract}
We propose two operations to prevent sharing in Haskell and demonstrate their use and usefulness.
\end{abstract}
...
Figure \ref{fig:stats} collects these statistics for the original and the modified code, each applied to an otherwise unreferenced tree first and then to a tree with a live reference, either unevaluated, unevaluated and wrapped in another thunk, partly evaluated and fully evaluated.
...
If, however, the type signature of \li-xs- is not given, then no unwanted sharing happens even with \ci!-O!. The infered, most general type of \li-xs- is polymorphic with type class constraints. This implies that additional parameters being passed under the hood, these successfully prevent sharing.
...
We extend Launchbury’s semantic for normalised lambda calculus with our two primitives:
\newcommand{\mdup}{\text{dup}}
\newcommand{\mdeepDup}{\text{deepDup}}
...
\newcommand{\sLet}[2]{\text{let}\ #1\ \text{in}\ #2}
\newcommand{\sred}[4]{#1 : #2 \Downarrow #3 : #4}
\newcommand{\sRule}[1]{\text{\textsf{\textsc{#1}}}}
...
\end{aligned}\\
\Gamma, \Delta, \Theta &\in \sHeap &&= \sVar \mapsto \sExp \\
z &\in \sVal &&\Coloneqq \lambda x . e
\end{alignat*}

In addition to the unmodified reduction rules \sRule{Lam}, \sRule{App}, \sRule{Variable} and \sRule{Let}, we add the two rules \sRule{Dup} and \sRule{DeepDup}, where $\fv e$ is the set of free variables of $e$.
\[
\begin{array}{cl}
\inferrule
{}
...
{\sred{\Gamma}{\sLet{x_1 = e_1,\ldots, x_n = e_n}e} x \Delta z}
& \sRule{Let}
\\\\
\inferrule
{\sred{\Gamma,x\mapsto e, x'\mapsto e} {x'} \Delta z}
...
{\begin{aligned}[b]
&x\mapsto e,\\
&y_i' \mapsto \sDeepDup y_i\\
&x'\mapsto [y_i'/y_i]e,
\end{aligned}}
} {x'} \Delta z \\ \{y_i\}_i = \fv e}
...
Launchbury defines a denotational semantics and proves his natural semantics to be correct with respect to the denotational semantics. Naturally we want to preserve this property. Our new primitives should be invisible to the denotational semantics, hence we extend the semantics function:
\begin{align*}
...
\end{theorem}
\begin{proof}
The proof is by induction on the deriviation, the two additional cases are:

\noindent\textbf{Case:} $\sDup x$

By induction, we know (i) $\dsem{x'}{\esem{\Gamma, x \mapsto e, x' \mapsto e} \rho} = \dsem{z}{\esem \Delta \rho}$ and (ii) $\esem{\Gamma, x \mapsto e, x' \mapsto e} \rho \le \esem \Delta \rho$.
...
&= \dsem{e}{\esem{\Gamma, x\mapsto e, x' \mapsto e}} && \text{($x'$ fresh)}\\
%&= \esem{\Gamma, x\mapsto e, x' \mapsto e}\rho\ x' \\
&= \dsem{x'}{\esem{\Gamma, x\mapsto e, x' \mapsto e}\rho} \\
&= \dsem{z}{\esem \Delta \rho} &&\text{(by (i))}
\end{align*}
as desired.

The second part follows from (ii) and that $x'$ is fresh:
\begin{align*}
\esem{\Gamma, x\mapsto e}\rho \le \esem{\Gamma, x\mapsto e, x'\mapsto e} \rho \le \esem{\Delta}\rho
\end{align*}

\noindent\textbf{Case:} $\sDeepDup x$

Let \mbox{$\Gamma' = \Gamma, x\mapsto e, x'\mapsto [y_i'/y_i]e, y_i' \mapsto \sDeepDup {y_i}$}.
By induction, we know (i) $\dsem{x'}{\esem{\Gamma'} \rho} = \dsem{z}{\esem \Delta \rho}$ and (ii) $\esem{\Gamma'} \rho \le \esem \Delta \rho$.

The newly introduced variables $y_i'$ have the same semantics as their original counterparts:
\begin{multline*}
\dsem{y'}{\esem{\Gamma'}\rho}
%= \esem{\Gamma'}\rho\ y
= \dsem{\sDeepDup y}{\esem{\Gamma'}\rho}
%\\
= \dsem{y}{\esem{\Gamma'}\rho}
%= \esem{\Gamma'}\rho\ y = \esem{\Gamma, x\mapsto e}\rho\ y
= \dsem{y}{\esem{\Gamma, x\mapsto e}\rho}.
\end{multline*}
This implies (iii) $\dsem{[y_i'/y_i]e}{\esem{\Gamma, x\mapsto e}\rho} = \dsem{e}{\esem{\Gamma'}\rho}$. Hence
\begin{align*}
&\phantom{{}={}}\dsem{\sDeepDup x}{\esem{\Gamma, x\mapsto e}\rho}\\
&= \dsem{x}{\esem{\Gamma, x\mapsto e}\rho}\\
&= \dsem{e}{\esem{\Gamma, x\mapsto e}\rho} &&\text{(by (iii))}\\
&= \dsem{[y_i'/y_i]e}{\esem{\Gamma'}\rho}\\
&= \dsem{x'}{\esem{\Gamma'}\rho}\\
&= \dsem{z}{\esem \Delta \rho} &&\text{(by (i))}
\end{align*}
and
\begin{align*}
...
\]
with $\fv{e'} \subset \{x_1',\ldots,x_n'\}$. If $\sred \Gamma e \Delta z$ and $z$ is a closed value ($\fv{z} = \emptyset$), then $\Gamma \le \Delta$.
\label{thm:deepdup}
\end{theorem}
This implies that any value on the heap $\Delta$ that was created during the evaluation of $e$ can be freed afterwards.

The theorem is an immediate consequence of the following lemma. TODO.
...
If we want to avoid this sharing, we need to prevent the original reference to be replaced by the indirection. We cannot change to code of the thunk, but we can copy the thunk, thus creating a new copy that is not referenced by other code, and then evaluate that. (TODO: picture?) The essence of the surprisingly simple code is listed in Figure \ref{fig:dupcode}; the closure to duplicate is passed in the register \ci-R1- and \ci-Hp- is the heap pointer which is increased by \ci-ALLOC_PRIM-.
...
deepDupClosure {
    ...
    for2:
    if(p < ptrs) {
	ap = Hp - bytes + WDS(1) + p * SIZEOF_StgAP + WDS(p);
