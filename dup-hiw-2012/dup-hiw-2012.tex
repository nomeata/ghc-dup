...
In addition to the unmodified reduction rules \sRule{Lam}, \sRule{App}, \sRule{Var} and \sRule{Let}, we add the three rules \sRule{Dup}, \sRule{Deep} and \sRule{DeepDeep} in Figure \ref{fig:semrules}. In these rules, $\fv e$ denotes the set of free variables of $e$, $\hat z$ is $z$ with all bound variables renamed to completely fresh variables and the variables $\fresh x$ and $\fresh y_1,\ldots,\fresh y_n$ are completely fresh.
...
\item the unguarded reachable set $\ur{\Gamma}{e}$ of an expression $e$ in a context $\Gamma$, mutually defined for all expressions as the smallest sets which fulfill the equation
\[
